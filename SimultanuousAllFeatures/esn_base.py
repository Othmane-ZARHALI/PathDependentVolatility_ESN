...
def build_esn_matrices(arch, inner_params):
    ...
    Build reservoir matrices given:
      ...
      inner_params — calibrated parameters dict (H, lam_lo, lam_hi,
                     az_lo, az_hi, rough_scale, zr_lo, zr_hi, m1, m2)

    The quadratic term in the pre-activation eta_t is r_t^T Q r_t with
    Q = (1/N_r) * (m1*I_{N_r} + m2*q q^T), so
        r_t^T Q r_t = (1/N_r) * (m1*||r_t||^2 + m2*(q^T r_t)^2),
    with m1, m2 both calibrated (no positive-definiteness constraint;
    either may be positive or negative). m1*I is an isotropic
    reservoir-energy channel; m2*qq^T is a second, independent channel
    responding specifically to the squared rough factor (q^T r_t)^2 --
    the same projection direction as the model's own linear rough term
    (rough_scale * q^T r_t), entering quadratically with its own
    coefficient.

    H is the ESN's own internal memory-kernel exponent (shapes
    q = lambdas^(0.5-H), the reservoir's own Volterra weighting),
    calibrated so the resulting simulated Hhat (measured via the
    log-vol structure function) matches each asset's own real target.
    """
    n_r  = int(arch["Nr"])
    n_z  = int(arch["Nz"])
    ...
    return dict(lambdas=lambdas, q=q,
                az=az, zz=zz, sz=sz,
...
def _sim_esn_with_params(seed, T, arch, matrices, b0_delta=0., scale=1., dt=1.0, alpha=1.0):
    """
    ...
    ALPHA FEEDBACK LAYER: the raw pre-activation variance,
    sigma_t^raw, feeds an EWMA recursion producing a 2nd, smoothed
    signal sigma_t^used = sqrt((1-alpha)*sigma_{t-1}^{used,2} +
    alpha*sigma_t^{raw,2}) -- a genuine feedback layer, since
    sigma_t^used depends recursively on its own previous value, not
    just on sigma_t^raw. Returns are generated from sigma_t^used
    (giving Taylor and kurtosis genuine multi-day persistence);
    Hurst, leverage, and Zumbach all read the unsmoothed sigma_t^raw
    (an EWMA is itself autocorrelated, and was found to inject a
    spurious dip-and-recovery shape into leverage's own
    corr(x_t,v_{t+L}) when read from a smoothed v). alpha=1.0
    recovers sigma_t^used == sigma_t^raw exactly (verified
    bit-identical), i.e. the single-signal model.
    """
    ...
    dx   = np.zeros(T);   dv = np.zeros(T);   dv_raw = np.zeros(T)
    ...
    for step in range(n_st):
        ...
        sp  = _sp(eta); sig_raw = math.sqrt(SIG_MIN**2 + sp**2); var_raw = sig_raw * sig_raw
        var_used = (1. - alpha) * var_used_prev + alpha * var_raw
        var_used_prev = var_used
        sig_used = math.sqrt(max(var_used, 1e-30))
        day = step // spd
        dx[day] += (MU_S - 0.5 * var_used) * dt + sig_used * sdt * eps
        dv[day] += var_used * dt
        dv_raw[day] += var_raw * dt
        r  = al * r + cl * eps
