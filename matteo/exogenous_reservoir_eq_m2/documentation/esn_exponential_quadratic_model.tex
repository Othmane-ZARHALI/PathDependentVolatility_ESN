...
\vspace{8mm}
{\Large\color{muted}
Current specification, physical stylised facts, and joint SPX--VIX calibration architecture\par}

\vspace{15mm}
\begin{tcolorbox}[colback=pale,colframe=blue,boxrule=0.8pt,arc=2mm]
\textbf{Purpose.}
This document consolidates the current candidate descended from ESN--A2. It states the model
...
measure, the same structural model should be capable of fitting SPX and VIX option surfaces.
...
The proposed model uses a structured bank of Ornstein--Uhlenbeck factors as a linear reservoir,
...
Gaussianity of the OU core between \(\Pp\) and \(\Qq\).
...
variance. The programme has two linked objectives:
...
\(\mathbb M\in\{\Pp,\Qq\}\), let \(W^{S,\mathbb M}\) drive the equity return. The structured
reservoir uses four driver groups:
...
where the superscripts \(F\) and \(J\) stand for feedback and fast-spike banks. The clustering
drivers \(B^{C}\) and optional orthogonal drivers \(B^{O}\) are independent of
...
For each bank \(b\in\{F,J,C,O\}\), define under \(\Pp\) the OU coordinates
...
at the stationary physical reference point. The direction \(Y\) carries the main
leverage--Zumbach mechanism; \(F\) controls the front end of volatility spikes.

\subsection{Layer 2: optional bounded echo-state layer}

On a time grid \(t_n=n\Delta t\), with \(0<\alpha\leq1\), the nonlinear reservoir is

\begin{equation}
Z_{n+1}
=(1-\alpha)Z_n
+\alpha\tanh\!\left(AZ_n+B\psi(X_n)+b\right),
\label{eq:esn-layer}
\end{equation}

where \(A,B,b\) are drawn or designed once and then held fixed. The feature map \(\psi\) may
contain selected linear, quadratic and cross-bank features. A sufficient contraction condition
for the echo-state property is
...
readout coefficient changes. The recommended first calibration sets the nonlinear loading
\(\delta\) below to zero. It is activated only if a physical-measure ablation demonstrates an
incremental gain that justifies losing Gaussian conditional-moment formulas.
...
\(q_O=0\), \(\delta=0\). Setting \(\beta_J=0\) exactly recovers the original Stage--3 M2
readout. Each shifted quadratic is deliberately parameterised without an additional linear
...
Let the full Brownian vector satisfy the deterministic Girsanov shift

\begin{equation}
\dd B_t^{\Pp}=\dd B_t^{\Qq}-\gamma(t)\dd t,
...
The unspanned components of \(\gamma\) are parsimonious variance-risk-premium parameters.
Because \(\gamma\) is deterministic, every OU bank remains Gaussian under \(\Qq\): only its
...
More explicitly, if \(\gamma_b(t)\) denotes the effective deterministic shift of the Brownian
driver of coordinate \((b,j)\), then
...
For the correlated banks, \(\gamma_b\) is the corresponding linear combination of the equity
and unspanned primitive-driver premia.
...
& Bank dimensions; OU rate grids; reservoir matrices \(A,B\); leak \(\alpha\);
...
A paired screen of 16 stable Stage--3 M2 configuration--scenario pairs, five fresh seeds and
...
pre-existing Stage--3 proxy gates passed in 67 of 80 evaluations; six of 16 configurations
