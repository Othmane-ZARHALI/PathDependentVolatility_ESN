...
THIS VERSION additionally: (a) reports S_avg, the bounded
Gaussian-kernel score, as the note's own primary reference metric
alongside Q (the unbounded quadratic sum of squares TRF actually
minimises) -- both are logged at every evaluation, per a follow-up
request; (b) includes plot_convergence(), reproducing the note's own
Figure 1 (raw S_avg per evaluation vs. its running best) directly
from history_s, with no extra simulation needed; (c) the final JSON
(market_final_trf_widegrid_gauss.json) adds n_evals and
primary_metric, and omits the large, purely-internal p_grid_points/
n_grid_points/status/message fields the earlier version saved.
"""
...
def plot_convergence(history_s, out_path="market_final_trf_convergence_widegrid_gauss.png"):
    """Reproduces the note's own Figure 1: raw S_avg per evaluation
    (gray) against the running-best S_avg (green step line), on the
    Gaussian-kernel score's own scale -- NOT Q, the quadratic sum of
    squares TRF actually minimises, which tracks S_avg's own minimum
    closely but is not itself bounded or as directly interpretable."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt

    n = len(history_s)
    running_best = np.minimum.accumulate(history_s)

    fig, ax = plt.subplots(figsize=(9, 6))
    ax.plot(range(1, n + 1), history_s, "o-", color="gray", alpha=0.6,
            ms=3, lw=0.8, label="raw evaluation")
    ax.plot(range(1, n + 1), running_best, "-", color="#1D9E75", lw=2.5,
            label=f"running best (start={running_best[0]:.2f}, "
                  f"min={running_best[-1]:.2f})")
    ax.set_xlabel("total number of iterations")
    ax.set_ylabel(r"$S_{avg}$ (Gaussian-kernel score)")
    ax.set_title(
        "Bounded LM convergence, WIDE grid, Gaussian-kernel score\n"
        r"$H\in[0.01, 0.3]$, $\lambda_{lo}\in[10^{-5}, 10^{-1}]$, "
        r"$\lambda_{hi}\in[1, 5]$, roughscale$\in[0.2, 0.7]$,"
        "\n"
        r"zr_lo$\in[0.01, 0.1]$, zr_hi$\in[0.1, 0.4]$ -- stopped at "
        f"{n} iterations"
    )
    ax.legend()
...
if __name__ == "__main__":
    ...
    json.dump({"history_q": history_q, "history_s": history_s,
               ...
               "n_evals": len(history_q),
               "p_bounds": P_BOUNDS_LOHI,
               "primary_metric": "S_avg (Gaussian kernel) -- reconnected per follow-up request"},
              open("market_final_trf_widegrid_gauss.json", "w"), indent=2)
    print("Saved market_final_trf_widegrid_gauss.json (checkpoint at", ckpt_path,
          "-- rerun same command to continue)")

    plot_convergence(history_s)
